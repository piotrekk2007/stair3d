...
import { LOAD_SOURCE_WARNING, UK_DOMESTIC_LOADS } from './loads.js';
...
function assumptionsFor(classId, wood, riserMaterial) {
  const list = [
    ...
    ...Object.values(UK_DOMESTIC_LOADS).map((l) => ({ text: `${l.label}: ${fmt(l.value)} (UK)`, source: l.ruleId, status: 'UK — do weryfikacji dla PL' })),
  ];
...
export function buildStructuralReport(models, options = {}) {
  ...
  return {
    ...
    material: wood,
    selfWeight,
    assumptions: assumptionsFor(classId, wood, options.riserMaterial),
    diagnostics,
  };
}
