...
function assumptionsFor(classId, wood, riserMaterial, config) {
  const list = [
    ...
    { text: 'Tralki: nośność nie jest liczona (decyzja użytkownika) — ich ciężar, jak słupków i poręczy, obciąża wangę, na której stoją; słupek stojący na podłodze obciąża podłogę', source: 'założenie programu', status: 'założenie' },
    ...
    { text: 'Obciążenie wangi: jej ciężar, połowa ciężaru i obciążenia każdego stopnia i podstopnia na niej, balustrada po jej stronie (tralki i słupki na desce, na której stoją; poręcz po długości); albo cała siła skupiona w środku rozpiętości', source: 'założenie programu', status: 'założenie' },
