...
export function renderProfileEditorSVG(views, options) {
  ...
  views.forEach((view, i) => {
    ...
    // housings ('closed' only) — WHERE a tread's slot is cut and how far it extends, INCLUDING its
    // nosing overhang (StringerSegmentConstructionGeometry.housings, see buildHousings()): the
    // whole point is to let the user see whether the nosing stays inside the board's own
    // silhouette or pokes out past it, which the plain "stopnie" boxes above (the structural
    // bearing, deliberately unaffected by nosing) cannot show. Still schematic in ONE respect: this
    // 2D side view has no third axis, so it cannot show how deep INTO the face the slot is routed —
    // only where it sits along the board and how tall/long it is.
